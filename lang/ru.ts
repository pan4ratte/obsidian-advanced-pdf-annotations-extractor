export default {
    ...
    SETTING_SECTION_SUBFOLDER_DESC: "Читает закладки самого PDF и кладёт каждую заметку в папку того раздела, где стоит её аннотация. Вложенность закладок повторяется вложенностью папок, а сами папки создаются внутри подпапки выше.",
