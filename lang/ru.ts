export default {
    ...
    SETTING_SORT_BY_TOPIC_DESC: "Распознаёт первую строку каждого комментария как тему: в общей заметке аннотации собираются по ней, а при извлечении в отдельные заметки она может стать названием заметки.",
    ...
    // Описания двух разделов ниже читаются в паре: они говорят, сколько заметок
    // получится и откуда возьмётся их имя.
    SECTION_SHARED_NOTES: "Извлечение в общие заметки",
    SECTION_SHARED_NOTES_DESC: "Одна заметка на все аннотации PDF — её имя задаётся шаблоном ниже.",
    ...
    SECTION_SEPARATE_NOTES_DESC: "Своя заметка на каждую аннотацию — её имя берётся из темы комментария или из шаблона ниже.",
